...
if __name__ == "__main__":
    ...
    # Pre-scan argv for --profile before any module that derives per-account file paths
    # (constants.py) gets imported, so DATA_DIR is computed correctly from the start.
    if "--profile" in sys.argv:
        _idx = sys.argv.index("--profile")
        if _idx + 1 < len(sys.argv):
            os.environ["TDM_PROFILE"] = sys.argv[_idx + 1]
    ...
    parser.add_argument(
        "--profile", default="", help="Run using an isolated account profile (multi-account)."
    )
