...
class Twitch:
    ...
    async def _run(self):
        ...
        # proactively re-validate the OAuth token every 30 minutes so an expired/invalid
        ...
        while True:
            ...
            if self._state is State.IDLE:
                ...
                if self.wanted_games:
                    ...
                    # nothing left to farm for now: run the configured power action once
                    if self.settings.auto_action is not PowerAction.NONE and not self._auto_action_done:
                        self._auto_action_done = True
                        self.print(_("status", "auto_action").format(action=self.settings.auto_action.value))
                        run_power_action(self.settings.auto_action)
    ...
    def _scheduler_allows_running(self) -> bool:
        if not self.settings.schedule_enabled:
            return True
        return within_window(self.settings.schedule_start, self.settings.schedule_end)

    async def _token_refresh_loop(self) -> None:
        # runs for the lifetime of the mining session, re-validating the token periodically
        # so an expired/revoked one is caught and refreshed (re-auth) before requests fail
        TOKEN_CHECK_INTERVAL = 30 * 60  # seconds
        while True:
