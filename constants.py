...
# Multi-account support: an optional profile name (set via --profile, or TDM_PROFILE env var
# read very early by main.py before this module's paths are computed) isolates all per-account
# data (settings, cookies, cache, stats, logs) into its own subfolder.
_PROFILE = os.environ.get("TDM_PROFILE", "").strip()
PROFILES_DIR = Path(WORKING_DIR, "profiles")
if _PROFILE:
    DATA_DIR = Path(PROFILES_DIR, _PROFILE)
    DATA_DIR.mkdir(parents=True, exist_ok=True)
else:
    DATA_DIR = WORKING_DIR
ACTIVE_PROFILE = _PROFILE or "default"
# Development paths
...
# Strings
WINDOW_TITLE = f"DropStream v{__version__}"
# Logging
LOGGING_LEVELS = {
    0: logging.ERROR,
    1: logging.WARNING,
